	...
	class WP_Sheet_Editor_Advanced_Filters {
		...
		public function add_settings_page_options( $sections ) {
			...
			$sections['misc']['fields'][] = array(
				...
				'title'      => __( 'What user roles are allowed to use the saved searches?', 'vg_sheet_editor' ),
				'desc'       => __( 'By default, only administrator can save and use the saved searches. Use this option to allow other roles to use the saved searches, but only administrators will be able to save new searches.', 'vg_sheet_editor' ),
				'type'       => 'new_select',
				'multi'      => true,
		...
		function add_favorite_search_fields_data( $data, $post_type ) {
			...
			if ( ! in_array( 'meta_query[0][source]', $data['favorite_search_fields'], true ) ) {
