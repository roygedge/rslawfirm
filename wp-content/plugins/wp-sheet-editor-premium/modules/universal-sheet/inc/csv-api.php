	...
	class WPSE_CSV_API {
		...
		function import_completed( $settings, $total, $processed ) {
			...
			if ( $writing_type === 'only_update' ) {
				WPSE_Logger_Obj()->entry( sprintf( 'The import is configured to only update existing rows in WordPress. All the rows that did not exist in WordPress and were skipped: %s', implode( ', ', $global_skipped_rows ) ), sanitize_text_field( $settings['wpse_job_id'] ) );
			} elseif ( $writing_type === 'only_new' ) {
				WPSE_Logger_Obj()->entry( sprintf( 'The import is configured to only create rows that don\'t exist in WordPress already. Rows that exist in WordPress and were skipped: %s', implode( ', ', $global_skipped_rows ) ), sanitize_text_field( $settings['wpse_job_id'] ) );
			}
		...
		function import_data( $settings ) {
			...
			if ( function_exists( 'WPSE_Logger_Obj' ) && ! empty( $settings['wpse_job_id'] ) ) {
				WPSE_Logger_Obj()->entry( sprintf( '%d rows will be processed in this batch', count( $rows ) ), sanitize_text_field( $settings['wpse_job_id'] ) );
			}
