...
if ( ! class_exists( 'WP_Sheet_Editor_Formulas' ) ) {
	...
	class WP_Sheet_Editor_Formulas {
		...
		public function prepare_formula( $formula, $original_formula, $column_key ) {
			$out       = array(
				'type' => '', // REPLACE, MATH, PHP
			...
			if ( strpos( $formula, '=REPLACE(' ) !== false ) {
				if ( strpos( $formula, ',' ) !== false ) {
					...
					$reg_exp = '/=REPLACE\(""(.*)"", ?""(.*)""\)/s';
					preg_match_all( $reg_exp, $formula, $result );
					preg_match_all( $reg_exp, $original_formula, $result_original_formula );

					// make replacement
					// If the search is current_value, assign the replace string directly
					if ( trim( $result_original_formula[1][0] ) === '$current_value$' ) {
						$sanitized_replace = VGSE()->helpers->sanitize_data_for_db(
							array(
								array(
									$column_key => $result[2][0],
								),
							),
							$post_type
						);
						...
						$out['set2'] = $sanitized_replace[0][ $column_key ];
					} else {
						if ( empty( $result[1][0] ) ) {
							$result[1][0] = '';
						}
						...
						$sanitized_search  = VGSE()->helpers->sanitize_data_for_db(
							array(
								array(
									$column_key => $result[1][0],
								),
							),
							$post_type
						);
						...
						$out['set1'] = $sanitized_search[0][ $column_key ];
						$out['set2'] = $sanitized_replace[0][ $column_key ];
		...
		public function apply_formula_to_data( $formula, $data, $post_id, $cell_args, $post_type ) {
			// Fix formula formatting
			$regex_flag       = self::$regex_flag;
			...
			$prepared_formula = $this->prepare_formula( $formula, $original_formula, $cell_args['key'] );
		...
		public function execute_formula_as_sql( $post_ids, $formula, $column, $post_type ) {
			...
			$prepared_formula  = $this->prepare_formula( $sanitized_formula, $sanitized_formula, $column['key'] );
		...
		public function bulk_execute_formula_ajax() {
			...
			$settings = array(
				...
				// Remove sanitization here, because it will be sanitized when the formula is parsed before being executed
		...
		public function _mark_all_items_for_bulk_edit_session( $total, $editor, $base_query, $bulk_edit_id, $post_type ) {
			$meta_table = $editor->provider->key === 'custom_table' || strpos( $editor->provider->key, 'hpos' ) !== false ? false : $editor->provider->get_meta_table_name( $post_type );
			...
			if ( $all_ids_to_mark ) {
				update_option( 'vgse_' . $bulk_edit_id, $all_ids_to_mark, false );
			}
		}
