	...
	class WP_Sheet_Editor_WooCommerce_Variations {
		...
		function save_columns_manager_options( $post_type ) {
			...
			foreach ( $_POST['column_settings'] as $column_key => $column_settings ) {
				if ( ! empty( $column_settings['allow_for_variations'] ) ) {
		...
		function render_field_to_allow_columns_for_variations( $option_key, $column, $post_type, $column_settings ) {
			...
			<div class="column-settings-field">					
				<label><input value="yes" <?php checked( in_array( $column['key'], $whitelisted_columns, true ) ); ?> type="checkbox" name="column_settings[<?php echo esc_attr( $column['key'] ); ?>][allow_for_variations]"> <?php _e( 'Allow to edit this column on variation rows?', 'vg_sheet_editor' ); ?></label>
		...
		public function add_column_settings_to_prepared_columns( $js_data, $post_type ) {
			if ( $post_type === 'product' ) {
				$js_data['default_column_data']['allow_for_variations'] = '';

				$whitelisted_columns = $this->_get_manually_whitelisted_columns();
				foreach ( $whitelisted_columns as $column_key ) {
					if ( isset( $js_data['prepared_columns'][ $column_key ] ) ) {
						$js_data['prepared_columns'][ $column_key ]['allow_for_variations'] = true;
					}
				}
			}
			return $js_data;
		...
		function _prepare_variations_data_for_copy( $product_variations, $variations_to_copy, $data ) {
			...
			foreach ( $product_variations as $variation ) {
				...
				$fields_to_remove = array( 'id', 'date_created', 'date_modified', 'permalink', 'sku', 'price', 'meta_data', 'global_unique_id' );
